...
export default function DetailView() {
  ...
  return (
    <View className="flex-1 bg-[#191A1F] p-4">
      ...
      {/* Scrollable Lyrics */}
      <ScrollView className="flex-1 mx-4" contentContainerStyle={{ paddingBottom: 24 }}>
        <Text
    ...
    if (router.canGoBack()) {
      ...
      router.push('/home'); // fallback về Home nếu không quay lại được
        ...
        <View className="absolute top-1/2 left-0 right-0 -translate-y-1/2 items-center">
          <Text className="text-white text-lg font-bold">{currentSong?.title}</Text>
          <Text className="text-[#CCCCCC] text-base">{currentSong?.artist}</Text>
        </View>
